...
def classify(path_text: str) -> tuple[str, str]:
    ...
    if normalized in L1_PREFIXES:
        return "L1", "changes behavior or role boundaries and should go through approval"
    if normalized.startswith("control/") or normalized.startswith("prompts/agents/"):
        return "L2", "core control or prompt change outside the safe allowlist"
